...
def init_ingredients_db(path: str):
    """
    Initialize the ingredients SQLite database with base schema and sample data.
    """
    if not os.path.exists(path):
        logger.info("[INIT] Creating and populating ingredients.db")
        conn = None
        try:
            ...
            """)
            # Insert base ingredients (example subset)
            ingredients = [
            ...
            c.executemany(
                "INSERT OR IGNORE INTO ingredients (name, type, common_uses) VALUES (?, ?, ?)",
                ingredients
            )
            ...
            c.executemany(
                "INSERT OR IGNORE INTO dishes (name, region, notes, prep_time_minutes) VALUES (?, ?, ?, ?)",
                dishes
            )
            conn.commit()
            logger.info("[INIT] ingredients.db created with base schema and sample data")
        except Exception as e:
            logger.error("Error initializing ingredients.db", exc_info=True)
            if conn:
                conn.rollback()
            raise
        finally:
        ...
        logger.info("[INIT] ingredients.db already exists")
